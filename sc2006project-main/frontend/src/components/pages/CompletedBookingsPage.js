...
function CompletedBookingsPage() {
  ...
  useEffect(() => {
    const fetchReservationData = async () => {
      try {
        const response = await fetch(`https://sc2006projectwebsitedeploy.onrender.com/api/reservation/myReservations/${user}`, {
          method: 'GET',
          headers: {
        ...
        if (!response.ok) {
          ...
          json.past_reservations.forEach(async (reservation) => {
            if (reservation.feedback_status === 'given') {
              try {
                const reviewResponse = await fetch(`https://sc2006projectwebsitedeploy.onrender.com/api/review/${reservation.reservation_id}_rvW`, {
                  method: "GET",
                  headers: {
                    'Content-Type': 'application/json',
                  },
